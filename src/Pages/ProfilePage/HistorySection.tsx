import { motion } from "framer-motion";
import { useEffect, useState } from "react";
import axios from "axios";
import { HistorySectionItem } from "./HistorySectionItem.tsx";
import { useSelector } from "react-redux";
import type { RootState } from "../../Store/Store.tsx";

interface HistorySectionProps {
  unwrapped: boolean;
  setUnwrapped: (v: boolean) => void;
  changes: boolean;
  setModalChildren: (v: string) => void;
  setTextMessage: (m: string) => void;
  setMessage: (message: boolean) => void;
  setModal: (v: boolean) => void;
  filters: {
    date_start: string;
    date_end: string;
    tags: string[];
  } | null;
}

export interface HistorySectionData {
  id: number;
  action_type: string;
  created_at: string;
  details: string;
  user: number;
  action_color: string;
}

export const HistorySection = ({
  unwrapped,
  setUnwrapped,
  changes,
  filters,
  setModalChildren,
  setModal,
  setTextMessage,
  setMessage,
}: HistorySectionProps) => {
  const animationVariants = {
    initial: { height: "35vh", width: "85%" },
    animate: { height: "86vh", width: "85%", marginBottom: "13px" },
  };
  const token = useSelector((state: RootState) => state.AuthSlice.token);
  const [data, setData] = useState<HistorySectionData[]>([]);
  const [disabled, setDisabled] = useState<boolean>(false);

  useEffect(() => {
    if (data.length <= 0) {
      setDisabled(true);
    } else {
      setDisabled(false);
    }
  }, [data, token]);
  useEffect(() => {
    const getData = async () => {
      try {
        if (filters) {
          const response = await axios.post<HistorySectionData[]>(
            "http://172.30.88.250:8000/users/logs/",
            filters,
            {
              headers: {
                Authorization: `Token ${token}`,
                "Content-Type": "application/json",
              },
            },
          );
          setData(response.data);
        } else {
          const response = await axios.get<HistorySectionData[]>(
            "http://172.30.88.250:8000/users/logs/",
            {
              headers: { Authorization: `Token ${token}` },
            },
          );
          setData(response.data);
        }
      } catch (err) {
        console.log(err);
      }
    };
    getData();
  }, [token, changes, filters]);

  const handleDelete = () => {
    axios
      .delete("http://172.30.88.250:8000/users/logs/", {
        headers: {
          Authorization: `Token ${token}`,
          "Content-Type": "application/json",
        },
      })
      .then((res) => {
        setData([]);
        console.log(res);
        setMessage(true);
        setTextMessage(res.data.message);
      })
      .catch((err) => {
        console.log(err);
      });
  };

  return (
    <motion.section
      className={"HistorySectionWrapper"}
      variants={animationVariants}
      animate={unwrapped ? "animate" : "initial"}
      transition={{ duration: 0.2 }}
    >
      <h2 className={"HistorySectionHeader"}>Logs History</h2>
      <section className={"HistorySectionTop"}>
        <svg
          onClick={() => {
            setModal(true);
            setModalChildren("Filter");
          }}
          className={"HistorySectionTopSvg"}
          width="30px"
          height="30px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M3 4.6C3 4.03995 3 3.75992 3.10899 3.54601C3.20487 3.35785 3.35785 3.20487 3.54601 3.10899C3.75992 3 4.03995 3 4.6 3H19.4C19.9601 3 20.2401 3 20.454 3.10899C20.6422 3.20487 20.7951 3.35785 20.891 3.54601C21 3.75992 21 4.03995 21 4.6V6.33726C21 6.58185 21 6.70414 20.9724 6.81923C20.9479 6.92127 20.9075 7.01881 20.8526 7.10828C20.7908 7.2092 20.7043 7.29568 20.5314 7.46863L14.4686 13.5314C14.2957 13.7043 14.2092 13.7908 14.1474 13.8917C14.0925 13.9812 14.0521 14.0787 14.0276 14.1808C14 14.2959 14 14.4182 14 14.6627V17L10 21V14.6627C10 14.4182 10 14.2959 9.97237 14.1808C9.94787 14.0787 9.90747 13.9812 9.85264 13.8917C9.7908 13.7908 9.70432 13.7043 9.53137 13.5314L3.46863 7.46863C3.29568 7.29568 3.2092 7.2092 3.14736 7.10828C3.09253 7.01881 3.05213 6.92127 3.02763 6.81923C3 6.70414 3 6.58185 3 6.33726V4.6Z"
            stroke="#000000"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>

        <button
          className={"HistorySectionTopButton"}
          disabled={disabled}
          onClick={handleDelete}
        >
          clear logs
        </button>
      </section>

      <div className="HistorySectionItemWrapper">
        {data?.map((el) => (
          <HistorySectionItem props={el} key={el.id} />
        ))}
      </div>
      <div className={"HistorySectionBottomSection"}>
        <button
          className={"HistorySectionButton"}
          onClick={() => setUnwrapped(!unwrapped)}
        >
          show more
        </button>
      </div>
    </motion.section>
  );
};
