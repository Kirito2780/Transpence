...
export const CustomForm = () => {
  ...
  const {
    ...
    register,
    formState: { errors },
  } = useForm<IFormValues>({ defaultValues: { icon: 1 } });
  ...
  const onSubmitCredit = async (data: IFormValues) => {
    try {
      ...
      formData.append("loan_type", "Other");
  ...
  return (
    <div>
      <Message setOpen={setOpen} open={open} error={error}>
        {message}
      </Message>
      <ButtonBack />
      <h2 className={"creditFormHeader"}>
        Create other Loan of debts dashboard
      </h2>
